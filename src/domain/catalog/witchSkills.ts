...
export const skillUsageHints: Partial<Record<WitchSkillId, string>> = {
 ...
 ignition: '使用建议：你的魔法是点火，整局只能使用一次，风险极高——若效果落空则整局白板。夜间使用有 90% 烧毁目标的一瓶药（可自选毒/解药，目标无药则落空）、10% 烧毁其全部魔女技；白天在完整票型公布后使用，有 90% 烧毁目标当天的投票（票作废）、10% 烧毁其全部魔女技。建议结合已公布票型选择目标，避免浪费。',
