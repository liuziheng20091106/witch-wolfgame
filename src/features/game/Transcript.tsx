...
export function Transcript({ observation }: TranscriptProps) {
  ...
  return <section className={styles.transcript} aria-labelledby="transcript-title">
    ...
    <div ref={viewportRef} className={styles.viewport} tabIndex={0} onScroll={(event) => {
      ...
      {observation.publicEvents.map((event) => <article key={event.id} className={`${styles.event} ${event.kind === 'trial-by-fire' || event.kind === 'result' ? styles.major : ''}`}>
        <span className={styles.icon}><EventIcon event={event} /></span>
        <div><span className={styles.meta}>第 {event.day} 天 · {event.phase}</span><p>{event.text}</p>
