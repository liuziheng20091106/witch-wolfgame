...
export function DecisionPanel({ observation, aiError, awaitingRetry, thinking, decisionError, onSubmit, onRetry, onLocal, onSettings }: DecisionPanelProps) {
  ...
  if (pending.schemaKey === 'voice-mimic') valid = !useSkill || (target !== '' && forgedSpeech.length > 0 && forgedSpeech.length <= 100 && mentionsRequired(forgedSpeech));
  ...
  return <section className={styles.panel} aria-labelledby="decision-title">
    ...
    <div className={styles.body}>
      ...
      {pending.schemaKey === 'voice-mimic' && useSkill && <>{targetControl(false)}<label className={styles.textarea}>伪造片段<textarea maxLength={100} value={forgedSpeech} onChange={(event) => setForgedSpeech(event.target.value)} /><span>{forgedSpeech.length}/100</span></label></>}
