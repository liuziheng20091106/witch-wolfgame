...
import { validateProviderResponse, validatePublicPayload } from '../server/gameProtocol.mjs';
import {
  INTERNAL_PATH,
  configPath,
  parseJsonBody,
...
function loadProviders(value) {
  ...
  return value.providers.map((provider, providerIndex) => {
    if (!provider || typeof provider !== 'object') throw new Error(`服务商 ${providerIndex} 配置无效`);
    if (typeof provider.name !== 'string' || !provider.name.trim()) throw new Error(`服务商 ${providerIndex} 缺少名称`);
...
function buildUpstreamPayload(input, provider) {
  const payload = {
...
async function callProvider(provider, apiKey, input, timeoutMs) {
  ...
  try {
    const response = await fetch(provider.endpoint, {
      ...
      body: JSON.stringify(buildUpstreamPayload(input, provider)),
    ...
    let value;
    try { value = JSON.parse(text); } catch { throw new Error(`服务商 ${provider.name} 返回非 JSON`); }
    if (!validateProviderResponse(value)) throw new Error(`服务商 ${provider.name} 响应结构或 JSON 内容无效`);
    return value;
  } finally {
...
class ProviderPool {
  ...
  async request(input) {
    ...
    for (let attempt = 0; attempt < this.maxAttempts; attempt += 1) {
      ...
      try {
        return await callProvider(slot.provider, slot.key, input, this.timeoutMs);
      } catch (error) {
        lastError = error;
...
export async function startProxyServer(configFile = process.env.MAJO_PROXY_CONFIG ?? resolve('proxy/proxy.config.json')) {
  ...
  const server = https.createServer({ ca, cert, key, requestCert: true, rejectUnauthorized: true, minVersion: 'TLSv1.3' }, async (request, response) => {
    ...
    try {
      ...
      if (!verifySignedRequest(password, request, url.pathname, body, nonceStore)) {
        return sendJson(response, 401, { error: 'invalid_internal_signature' });
      }
      ...
      sendJson(response, 200, await pool.request(payload));
    } catch (error) {
      const status = Number.isInteger(error.statusCode) ? error.statusCode : 502;
      sendJson(response, status, { error: status === 502 ? 'upstream_unavailable' : 'bad_request', message: error.message });
    }
  });
  server.on('close', () => clearInterval(pruneTimer));
  await new Promise((resolvePromise, reject) => {
    server.once('error', reject);
    server.listen(config.listen.port ?? 34023, config.listen.host ?? '0.0.0.0', resolvePromise);
  });
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(import.meta.filename)) {
  startProxyServer().catch((error) => { console.error(error); process.exitCode = 1; });
}
