...
try {
  ...
  const brokenRoom = structuredClone(validRoom);
  brokenRoom.roomCode = 'ERR234';
  brokenRoom.game.pendingDecision = { id: 'broken-pending' };
  const brokenStateFile = join(temp, 'broken-rooms.json');
  await writeFile(brokenStateFile, JSON.stringify([brokenRoom]), 'utf8');
  const brokenPort = port + 3;
  ...
  assert.equal(broken.exitCode, null, 'AI 驱动异常不得终止多人服务');
  assert.match(brokenErrors, /multiplayer_ai_drive_error/);
  broken.kill('SIGTERM');
