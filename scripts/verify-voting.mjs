...
console.log('=== 1. 首轮投票秘密提交并统一揭晓 ===');
{
  ...
  check('首轮揭票事件包含全部结构化票型', Array.isArray(reveal[0]?.data.voteRecords) && reveal[0].data.voteRecords.length === expectedVoterCount);
  check('首轮揭票事件包含结构化提交票数汇总', Array.isArray(reveal[0]?.data.submittedVoteTally));
  ...
  check('首轮提交票数汇总覆盖全部投票', tallyTotal === expectedVoterCount, `预期=${expectedVoterCount}，汇总=${tallyTotal}`);
  ...
  check('统一揭票后玩家可见完整首轮票型', revealedObservation.currentVotes.length === expectedVoterCount);
}
...
console.log('\n=== 3. 平票重投继续封闭并统一揭晓 ===');
{
  ...
  const expectedVoterCount = livingRealVoterCount(game);
  game.currentVotes = game.players.filter((player) => player.alive).map((player, index) => {
    let targetPlayerId = 0;
    if (player.id === 0 || (player.id !== 1 && index % 2 === 0)) {
      targetPlayerId = 1;
    }
    return { voterPlayerId: player.id, targetPlayerId, round: 1 };
  });
