...
try {
  ...
  ({ getDayIgnitionDecision, getHealingDecision } = await server.ssrLoadModule('/src/domain/skills/nightSkills.ts'));
...
console.log('=== 狼人治愈本队狼刀 ===');
{
  ...
  const oversizedPending = { ...pending, description: '投'.repeat(PROMPT_LIMITS.actionDescriptionMaxLength + 32) };
  const oversizedMessages = promptForPending(state, oversizedPending);
  const oversizedPayload = JSON.parse(oversizedMessages[1].content);
  assert.equal(oversizedPayload.action.description.length, PROMPT_LIMITS.actionDescriptionMaxLength);
  assert.deepEqual(validateGamePrompt(oversizedMessages), { ok: true }, '超长行动说明必须在发送前截断');
  const ignitionState = createGame({ mode: 'spectator', humanCharacterId: null, seed: 419384535, playerCount: 12, selectedCharacterIds: [] });
  ignitionState.day = 1;
  ignitionState.phase = 'voting';
  const ignitionSkill = ignitionState.skillInstances.find((entry) => entry.definitionId === 'ignition');
  assert.notEqual(ignitionSkill, undefined);
  ignitionSkill.status = 'ready';
  delete ignitionSkill.data.lastOfferedKey;
  ignitionState.currentVotes = ignitionState.players.slice(0, 11).map((player, index) => ({ round: 1, voterPlayerId: player.id, targetPlayerId: ignitionState.players[(index + 1) % 12].id }));
  const ignitionPending = getDayIgnitionDecision(ignitionState);
  assert.notEqual(ignitionPending, null);
  assert.ok(ignitionPending.description.length > PROMPT_LIMITS.actionDescriptionMaxLength, '回归样例必须覆盖超长点火说明');
  assert.deepEqual(validateGamePrompt(promptForPending(ignitionState, ignitionPending)), { ok: true }, '长票型点火提示必须通过 Node 校验');
