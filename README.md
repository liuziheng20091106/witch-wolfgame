...
支持 6–14 人的 AI 狼人杀网页游戏。开局可指定人数与出庭角色，使用默认或自定义版型，并选择随机分配或顺序选职。基础职业池含 11 种：狼人、守卫、猎人、预言家、女巫、村民、白狼王（狼）、隐狼（狼）、呆头鹅（中立）、守墓人（好人）和暗杀者（狼）。角色人格、基础职业与魔女技互相独立。

当前客户端与提示词协议版本为 `2.4.0`。涉及发布或协议变更时，检查构建注入的客户端版本及目标部署的兼容范围；主后端和代理的 `acceptedClientVersions` 是允许列表，可包含多个受支持版本。普通功能、修复、重构或文档 PR 不要求升级版本或同步所有版本字符串。

## 功能

- 6–14 人可配置阵容：可随机或手动选择出庭角色，支持全自动 AI 观战及用户加入一个席位
- 默认版型：保留每档人数的原有配置，其中守卫仅 9+ 人局、隐狼仅 13/14 人局、呆头鹅在 9/12 人局；自定义版型可调整这些职业的出场人数
- 自定义版型与两种身份分配方式支持本地真人、全自动 AI 观战和多人房间，连续审判保留配置
...
- 职业与魔法图鉴：准备区与对局内均可查阅 11 个基础职业和 14 项魔女技，展示阵营、胜利条件、技能效果、初始持有者、发动时机与使用次数，支持搜索和筛选
...
准备区可增减各职业人数，或恢复当前人数的默认版型。职业总数须等于席位数；狼人、村民可重复，其他职业各最多 1 名，中立最多 1 名。狼队人数须在 `max(1, floor(N/3)-1)` 至 `floor(N/3)+1` 之间，且开局好人人数多于狼队。这些是配置护栏，不代表自定义组合已经过平衡验证。新增职业通过自定义版型加入。

- **随机分配**：使用带种子的随机数，将配置好的职业池分配给各席位。
- **顺序选职**：开局生成独立的秘密选职顺序，每人从剩余职业中的最多 3 个不同候选选择，或随机抽取一个剩余职业。接近末尾时候选可能少于 3 个；随机选项可抽到候选外的剩余职业。最终职业数量与配置完全一致。普通玩家只能看到自己的选择，狼队身份在选职结束后共享；观战者可看到已确定的职业。支持中途保存与多人重连恢复。
- **守墓人**：每夜查验一名已死亡者的当前职业，结果仅自己可见；隐狼显示为村民。没有死者时自动跳过。
- **暗杀者**：狼队职业，每局一次，在全部发言结束、投票前猜测一名其他存活者的当前职业。猜对则目标死亡，猜错则自己死亡；暂不发动不消耗机会。漂浮生效的目标不可选，死亡沿用既有结算与回溯机制。

顺序选职参考鹅鸭杀的轮抽机制，按本作固定职业池与回合制适配；暗杀者按本作设计限制为整局一次。
